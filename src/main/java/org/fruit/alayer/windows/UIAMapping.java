...
public class UIAMapping implements IUIAMapping {
    ...
    private BiMap<Long, Tag<?>> patternPropertyMapping = HashBiMap.create();

    public UIAMapping() {
        patternPropertyMapping.put(org.fruit.alayer.windows.Windows.UIA_IsAnnotationPatternAvailablePropertyId, UIATags.UIAIsAnnotationPatternAvailable);
    ...
    public <T> Optional<Tag<T>> getMappedStateTag(Tag<T> mappedTag) {
        var mapping =  (Tag<T>) stateTagMappingWindows.getOrDefault(mappedTag, null);
        if (mapping == null){
